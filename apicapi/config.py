...
apic_new_opts = [
    # REVISIT(ivar): use_apic_options was born already deprecated, for it will
    # be removed together with the ml2_cisco_apic group options. We need this
    # option (instead of just opportunistically use new-style config) because
    # we can't tell whether a new-style option was actually set by the user
    # or it's just the default value being loaded.
    cfg.BoolOpt('use_apic_options', default=False,
                deprecated_for_removal=True,
                help=("Decide whether new-style configurations should be "
                      "used by APICAPI.")),
]
...
cfg.CONF.register_opts(apic_opts + apic_new_opts + apic_opts_from_ml2, "apic")
